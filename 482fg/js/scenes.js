...
phina.define('GameTitleScene', {
  ...
  init: function (options) {
    ...
    Label({
      text: '1P: A/D 移動  W ジャンプ  S しゃがみ  J弱 K強 Lキック\n2P: ←/→ 移動  ↑ジャンプ  ↓しゃがみ  O弱 P強 Iキック\n必殺  ↓↘→+弱/強  波動    →↓↘+弱/強  昇龍    ↓↙←+キック  竜巻',
      fontSize: 18,
      fill: '#ddd',
      align: 'center',
  ...
  update: function (app) {
    if (app.keyboard.getKeyDown('space')) this.exit();
  },
});
...
phina.define('VersusScene', {
  ...
  init: function (options) {
    ...
    Label({ text: '1P ' + p1Data.name, fontSize: 14, fill: '#fff' }).addChildTo(this).setPosition(24, 18);
    Label({ text: '2P ' + p2Data.name, fontSize: 14, fill: '#fff' }).addChildTo(this).setPosition(SCREEN_WIDTH - 24, 18);
